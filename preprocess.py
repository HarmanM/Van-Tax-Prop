...
def mergePropTax(prop2016subset, prop2011subset):
    ...
    mergedPropertyDF['CURRENT_LAND_VALUE'] = mergedPropertyDF['CURRENT_LAND_VALUE_y'] - mergedPropertyDF[
        'CURRENT_LAND_VALUE_x']
    mergedPropertyDF['CURRENT_IMPROVEMENT_VALUE'] = mergedPropertyDF['CURRENT_IMPROVEMENT_VALUE_y'] - mergedPropertyDF[
        'CURRENT_IMPROVEMENT_VALUE_x']
...
def mergePropTax_2006_2016(prop2016subset, prop2006subset):
    ...
    # getting the delta values by subtracting the two datasets (Note: the 2006-2013 property tax report dataset does not have any previous land value
    # or previous improvement value entries
    mergedPropertyDF['CURRENT_LAND_VALUE_DELTA'] = mergedPropertyDF['CURRENT_LAND_VALUE_y'] - mergedPropertyDF[
        'CURRENT_LAND_VALUE_x']
    mergedPropertyDF['CURRENT_IMPROVEMENT_VALUE_DELTA'] = mergedPropertyDF['CURRENT_IMPROVEMENT_VALUE_y'] - \
                                                          mergedPropertyDF['CURRENT_IMPROVEMENT_VALUE_x']
    mergedPropertyDF.reset_index(inplace=True, drop=True)
    return mergedPropertyDF
...
def addCensus(data, census):
    for index_count in range(len(census)):
        feature_name = census.iloc[index_count]['Unnamed: 0']
        feature_array = []
        ...
        data[feature_name] = feature_array
        print(index_count)
    return
    ...
    for i in old_data:
        if (old_data[i].dtype == 'object' or len(old_data[i].unique()) < 20):
            ...
            for j in range(len(encode.columns)):
                ...
                data_subset.insert(counter + j, column=encoded_columns_name, value=encode[encoded_name],
...
def mergePropTax2006_2011(prop2011subset):
    ...
    subset_2011 = prop2011subset.drop(prop2011subset.loc[~prop2011subset['REPORT_YEAR'].isin([2011])].index,
                                      inplace=False)
    subset_2006 = prop2011subset.drop(prop2011subset.loc[~prop2011subset['REPORT_YEAR'].isin([2006])].index,
                                      inplace=False)
...
def census2006_2016(census2006, census2016):
    ...
    deltaCensus = pd.DataFrame(
        columns=['Arbutus-Ridge', 'Downtown', 'Dunbar-Southlands', 'Fairview', 'Hastings-Sunrise',
                 'Kensington-Cedar Cottage', 'Kerrisdale'
            , 'Killarney', 'Kitsilano', 'Marpole', 'Mount Pleasant', 'Oakridge', 'Renfrew-Collingwood', 'Riley Park',
                 'Shaughnessy', 'South Cambie',
...
def deltaCensus2006_2016(census2006, census2016):
    ...
    deltaCensus.loc['deltaPersonsPrivateHousehold'] = persons_private_household2016.iloc[0] - \
                                                      persons_private_household2006.iloc[0]
...
addCensus(merged_2006_2016_frame, censusDF)

print(merged_2006_2016_frame.head(100).to_string())
# Drop any unnecessary columns
dropColumns(merged_2006_2016_frame)
newdf = pd.get_dummies(merged_2006_2016_frame, columns=['ZONE_CATEGORY', 'Geo Local Area', 'LEGAL_TYPE'],
                       prefix=['ZONE_CATEGORY', 'REGION', 'LEGAL_TYPE'])
newdf.to_csv('preprocessed_complete_2006_2016.csv')
